...
angular.module('bootstrapLightbox').provider('Lightbox', function () {
  ...
  this.calculateModalDimensions = function (dimensions) {
    ...
    // 66px = 32px as above
    //        + 34px outer height of .lightbox-nav
    // 130px is a test data for add gps info in lightbox should be 66px
    var height = Math.max(200, dimensions.imageDisplayHeight + 130);
  ...
  return {
    'link': function (scope, element, attrs) {
      ...
      scope.$watch(function () {
        return attrs.lightboxResize;
      }, function (data, src) {
        if (data == "true") {
          // resize the img element and the containing modal
          resize();
          scope.lightboxResize = false;
        }
      });
